...
import TronLib from '../../../../src/lib/networks/tron.js'
...
describe('#Tron', () => {
  ...
  before(async () => {
    ...
    uut = new TronLib({ useCases, wlogger: { info: () => { }, error: () => { } }, NetworksData })
    uut.TronWeb = class TronWeb {
      constructor () {
        this.trx = {
          getCurrentBlock: () => { return { number: 1234567890 } },
          getBalance: () => { return { number: 1234567890 } },
          send: () => { return { result: true } }
        }
        this.getBandwidth = () => { return { number: 1234567890 } }
        this.getBandwidthPrices = () => { return { number: 1234567890 } }
        this.address = {
          fromPrivateKey: () => { return { number: 1234567890 } }
        }
        this.transactionBuilder = {
          sendTrx: () => { return { number: 1234567890 } }
        }
      }
    }
    uut.rate = {
  ...
  describe('#start', () => {
    it('should start tron network', async () => {
      sandbox.stub(uut, 'verifyConnection').resolves()

      const res = await uut.start('trx')
      assert.isTrue(res)
    })
    it('should throw error if tronKey is not provided', async () => {
      uut.tronKey = null
      try {
        await uut.start('trx')
        assert.fail('Unexpected code path')
      } catch (error) {
        assert.include(error.message, 'Tron Api Key must be provided!')
      }
    })
    it('should throw error if chainKey is not provided', async () => {
      try {
        ...
        assert.include(error.message, 'chainKey must be provided when start tron!')
      }
    })
  ...
  describe('#verifyConnection', () => {
    it('should verify connection', async () => {
      sandbox.stub(uut.provider.trx, 'getCurrentBlock').resolves({
        number: 1234567890
      })
    ...
    it('should throw error if connection is not verified', async () => {
      sandbox.stub(uut.provider.trx, 'getCurrentBlock').rejects(new Error('Connection failed'))
      try {
  ...
  describe('#getBalance', () => {
    it('should get balance', async () => {
      sandbox.stub(uut.provider.trx, 'getBalance').resolves(1000000000000000000)
  ...
  describe('#send', () => {
    it('should send', async () => {
      ...
      sandbox.stub(uut.provider.trx, 'send').resolves({
        result: true,
        transaction: {
    ...
    it('should handle error if transaction result is false', async () => {
      try {
        sandbox.stub(uut.provider.trx, 'getBalance').resolves(1000000000000000000)
        sandbox.stub(uut.feeLib, 'getFee').resolves({
          fee: 0.001,
        ...
        await uut.send('0x0000000000000000000000000000000000000000', 100000000000000n, 'privateKey')
        assert.fail('Unexpected code path')
      } catch (error) {
        assert.include(error.message, 'Transaction Error')
      }
    })
    it('should throw error if address is not provided', async () => {
      try {
